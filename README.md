...
- You should generate `fsbl.elf` using `generate_fsbl.tcl` in the following section

### <a name="fsbl"></a> Build fsbl.elf
`fsbl.elf` in the current vivado project doesn't support QSPI boot. 

generate `fsbl.elf` with following method:
...
# Rocky 8.10
sudo dnf install xorg-x11-utils xorg-x11-apps
